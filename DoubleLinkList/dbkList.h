...
void lnkList<T>::push_back(T val) {
    Node<T> *temp, *pre_pos;  // temp为插入节点，pre_pos为插入位置前驱节点
    pre_pos = tail->pre;  // 获取末节点
    temp = new Node<T>(val, tail, pre_pos);  // temp后驱更新为tail
    tail->pre = temp;  // 更新tail前驱
    pre_pos->next = temp;  // 前末节点的后驱更新为新插入节点
    ++len;  // 长度+1
    cout << "追加成功" << endl;
}
...
void lnkList<T>::insert(int p, T val) {
    if (p < 0 || p > size()) {  // 不能往头结点前或尾节点后插值
        ...
        Node<T> *pre_pos, *temp;  // temp为待插入节点，pre_pos为插入位置的前一个节点

        pre_pos = getNode(p-1);  // 获取插入位置的前一个节点
        temp = new Node<T>(val, pre_pos->next, pre_pos);  // temp的下一个节点设置为pre_pos的下一个节点，上一个节点为pre_pos
        pre_pos->next = temp;  // pre_pos的后驱指针更新为temp
        (temp->next)->pre = temp;  // 原pre_pos的后驱节点的前驱指针更新为temp
        ++len;  // 长度+1
        cout << "追加成功" << endl;
    }
}
...
void lnkList<T>::erase(int p) {
    ...
    if(p < 0 || p >= size()){
        ...
        Node<T> *temp, *pre_pos;  // temp为要删除的节点，pre_pos为要删除节点的前一个节点

        temp = getNode(p);  // 获取删除节点
        pre_pos = temp->pre;
        pre_pos->next = temp->next;  // pre_pos的后驱更新为temp的后驱
        (temp->next)->pre = pre_pos;  // temp的后驱的前驱指针更新为pre_pos
        delete temp;
        --len;  // 长度-1
        cout << "删除成功" << endl;
    }
...
void lnkList<T>::pop_back() {
    ...
    Node<T> *pre_pos, *temp;  // pre_pos为末节点前驱节点，temp为末节点
    temp = tail->next;  // 获取末节点
    pre_pos = temp->pre;  // 获取末节点前驱节点
    pre_pos->next = tail;  // 前末节点前驱节点更新为末节点
    tail->pre = pre_pos;  // 末节点更新为pre_pos
    delete temp;
    --len;
    cout << "删除成功" << endl;
}
...
void lnkList<T>::pop_front() {
    ...
    (temp->next)->pre = head->next;  // 更新首节点后驱的前驱指针
    delete temp;
    --len;
    cout << "删除成功" << endl;
}
...
void lnkList<T>::push_front(T val) {
    ...
    temp = new Node<T>(val, head->next, head);
    (head->next)->pre = temp;  // 更新首节点后驱节点的前驱指针
    head->next = temp;  // 更新头节点后驱指针
    ++len;
    cout << "插入成功" << endl;
}
...
void lnkList<T>::init() {
    ...
    head->next = tail;  // 头结点后驱为尾节点
    head->pre = nullptr;  // 头节点前驱为空
    tail->next = nullptr;  // 尾结点后驱为空
    tail->pre = head;  // 尾节点前驱为头节点
}
